...
public class XBullets {
    public static BulletType QuickTypeBullet, assaultTypeBullet, assaultTypeBulletBig, guardianTerrorBlast, blastTypeBullet, emberBulletType;
 static {
    ...
    assaultTypeBullet = new BulletType(4f, 150){{
     ...
     guardianTerrorBlast = new BulletType(5f, 2400){{
         trailChance = 100f;
         lifetime = 40f;
         hitSize = 32;
         drawSize = 120f;
         ...
         //buildingDamageMultiplier = 0.2f;
         hitSound = Sounds.explosion;
         hitEffect = new Effect(50f, 120f, e -> {
             float rad = 64f;
             e.scaled(7f, b -> {
                 color(XColors.guardianFront, b.fout());
                 Fill.circle(e.x, e.y, rad);
             });

             color(XColors.guardianFront);
             stroke(e.fout() * 3f);
             Lines.circle(e.x, e.y, rad);
             ...
             color();
             Fill.circle(e.x, e.y, 3f * e.fout());
             Drawf.light(e.x, e.y, rad * 1.6f, XColors.guardianFront, e.fout());
         });

         trailEffect = new Effect(1f, 36f, e->{
             float range = 4f;
             float blinkScl = 20f;
             int sectors = 8;
             float effectRadius = 16f, blinkSize = 0.1f;
             float rotateSpeed = 8f;
             float sectorRad = 0.14f;
             Draw.z(layer);
             Draw.color(XColors.guardianBack);
             float orbRadius = effectRadius * (1f + Mathf.absin(blinkScl, blinkSize));
             ...
             Lines.stroke((0.7f + Mathf.absin(blinkScl, 0.7f)), XColors.guardianBack);
             ...
             Drawf.light(e.x, e.y, range * 1.5f, XColors.guardianBack, 0.8f);
